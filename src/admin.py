...
async def admin_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "admin_start":
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=messages.ADMIN_SCENARIO_INFORM[user_language]
        )
    else:
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=messages.EXAM_ID_CREATION[user_language] % generated_exam_id
    )
...
async def admin_print_exam_registration_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                messages.EXAM_REGISTRATION_FINISH_PLUS_STUDENTS_LIST[user_language], callback_data="admin_student_list"
            ),
        ]
    ]
    ...
    await context.bot.send_message(
        ...
        text=messages.EXAM_REGISTRATION_FINISHING_DESCRIPTION[user_language],
...
async def admin_exam_registration_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not student_list_for_exam:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=messages.EXAM_REGISTRATION_FINISHING_NO_STUDENTS_ERROR[user_language]
        )
        return await admin_print_exam_registration_finish(update, context)
    await context.bot.send_message(chat_id=update.effective_chat.id, text=", ".join(student_list_for_exam))

    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=messages.EXAM_STARTING_PLUS_EXAM_FINISH_COMMAND[user_language]
    )
...
async def admin_finish_exam_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text=messages.EXAM_FINISH_PLUS_RESULTS_AGGREGATION[user_language]
    )
    ...
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=messages.EXAM_PER_STUDENT_RESULTS[user_language],
        reply_markup=reply_markup,
    )
