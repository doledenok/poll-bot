...
async def admin_exam_registration_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data != "admin_student_list":
        ...
        return
    student_list_for_exam = "Student list is still empty :("
    await context.bot.send_message(chat_id=update.effective_chat.id, text=student_list_for_exam)
